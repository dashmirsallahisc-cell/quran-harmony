...
export function SurahCard({ surah }: Props) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-2xl border border-border/40 bg-surface/60 p-3 shadow-card transition-base ${isCurrent ? "border-gold/60 shadow-glow" : "hover:border-gold/30"}`}>
      ...
      <DropdownMenu onOpenChange={(o) => o && downloaded === null && checkDownloaded()}>
        <DropdownMenuTrigger asChild>
          <button aria-label="More" className="grid h-9 w-9 place-items-center rounded-full text-muted-foreground transition-base hover:bg-muted/50">
            <MoreVertical className="h-5 w-5" />
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-48">
          <DropdownMenuItem onClick={() => player.toggleFavorite(surah.number)}>
            <Heart className={`mr-2 h-4 w-4 ${isFav ? "fill-gold text-gold" : ""}`} />
            {isFav ? lang.ui.remove : lang.ui.favorites}
          </DropdownMenuItem>
          {downloaded ? (
            <DropdownMenuItem onClick={handleRemove}>
              <Trash2 className="mr-2 h-4 w-4" /> {lang.ui.remove}
            </DropdownMenuItem>
          ) : (
            <DropdownMenuItem disabled={downloading} onClick={handleDownload}>
              <DLIcon className="mr-2 h-4 w-4" />
              {downloading ? `${lang.ui.download}...` : lang.ui.download}
            </DropdownMenuItem>
          )}
        </DropdownMenuContent>
      </DropdownMenu>
