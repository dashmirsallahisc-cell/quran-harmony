  ...
  if (!p.surah) {
    return (
      <div className="grid min-h-[60vh] place-items-center px-6 text-center">
        <div>
          <div className="mx-auto mb-6 grid h-24 w-24 place-items-center rounded-full bg-gradient-gold text-3xl text-gold-foreground shadow-glow">
            ۞
          </div>
    ...
    <div className="flex flex-col gap-5 px-4 pt-4 pb-6">
      ...
      <div className="mx-auto grid h-56 w-56 place-items-center rounded-full bg-gradient-gold shadow-glow">
        <div className="grid h-52 w-52 place-items-center rounded-full bg-background">
          <span
            className={`font-arabic text-7xl text-gold ${p.isPlaying ? "animate-spin-slow" : ""}`}
          >
            ۞
          </span>
        </div>
      ...
      <div className="flex items-center justify-center gap-6">
        <button
          onClick={() => p.toggleFavorite(p.surah!.number)}
          className="grid h-11 w-11 place-items-center rounded-full text-foreground hover:bg-muted/40 transition-base"
        >
          <Heart className={`h-5 w-5 ${isFav ? "fill-gold text-gold" : ""}`} />
        </button>
        <button
          onClick={p.prev}
          className="grid h-12 w-12 place-items-center rounded-full text-foreground hover:bg-muted/40 transition-base active:scale-90"
        >
          <SkipBack className="h-7 w-7 fill-current" />
        </button>
        <button
          onClick={p.toggle}
          className="grid h-16 w-16 place-items-center rounded-full bg-gradient-gold text-gold-foreground shadow-glow active:scale-90 transition-base"
        >
          {p.isPlaying ? (
        ...
        <button
          ...
          aria-label="Autoplay"
          className={`grid h-11 w-11 place-items-center rounded-full transition-base hover:bg-muted/40 ${p.autoplay ? "text-gold" : "text-muted-foreground"}`}
        >
          ...
          <div className="space-y-4">
            {arabic.ayahs.slice(0, 30).map((a, i) => (
              <div key={a.number} className="border-b border-border/30 pb-3 last:border-0">
                <div className="mb-2 flex items-center gap-2">
                  <span className="grid h-6 w-6 place-items-center rounded-full bg-gold text-[11px] font-bold text-gold-foreground">
                    {a.numberInSurah}
                  </span>
                </div>
            ...
            {arabic.ayahs.length > 30 && (
              <p className="text-center text-xs text-muted-foreground">
                + {arabic.ayahs.length - 30} more verses
              </p>
            )}
          </div>
        </div>
